...
$(document).ready(function(){
	...
	$('#shaOneUpdate').click(function(){
			...
			$('#sha_email_error, #sha_dob_error, #sha_arrival_date_error').hide();
			
			if(sha_fname=='' || sha_lname=='' || sha_gender=='' || sha_dob=='' || (sha_dob!='' && !isValidDate(sha_dob)) || sha_email=='' || !isValidEmailAddress(sha_email) || sha_accomodation=='' || ($study_group_check.length>0 && $study_group_check.is(':checked') && sha_study_group=='') || (sha_arrival_date!='' && !isValidDate(sha_arrival_date)))
			{
				if(sha_fname=='')	
