...
$bookingStatusList=bookingStatusList();
			 ...
			 $row6 .='';
						  ...
						  <td><strong>Booking id: <?= $val['bookid']?></strong><br>Duration: <span id="viewBooking_duration"><?=$st.$endd ?></span><br><?=  $accomodationTypeList[$val['accomodation_type']]?><?php if($val['serviceOnlyBooking']=='0'){echo ' (Room '.$bookingBedNumber.')';} ?></td>
						  <td>
                          <?php if($val['serviceOnlyBooking']=='0'){?>
                          	<a href='<?= site_url() ?>hfa/application/<?=  $val['host'] ?>' target="_blank"><?=  ucfirst($val['hlname']).' Family' ?></a><br><a class="mailto" href="mailto:<?=$val['hemail']?>"><?=   $val['hemail']  ?></a><br><?=  $row1 ?>
                          <?php } else{echo 'n/a';} ?>
                          </td>
