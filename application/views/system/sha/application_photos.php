<div class="col-md-3">
    ...
    <div class="panel panel-profile panel-bluegraylight">
        ...
        <div class="panel-body">
            <form action="<?=site_url()?>sha/application_image_upload" id="hf-photos-form" class="dropzone">
    ...
    <div id="changeProfilePicDiv" <?php if(empty($photos)){?>style="display:none;"<?php } ?>>
        <div class="panel panel-profile panel-bluegraylight"  id="">
            <div class="panel-heading">
                <h2>STUDENT PROFILE PHOTO</h2>
            </div>
            <div class="panel-body">
                  <button class="btn btn-primary btn-raised btn-sm" data-toggle="modal" data-target="#profilePicModel"  onclick="selectProfilePhotoPopContent(<?=$formOne['id']?>,'sha');" id="selectProfilePhotoBtn"><?=$selectBtnText?> profile photo</button>
            </div>
        </div>
    </div>
</div>
...
<div class="col-md-9 panel-profile">
    <div class="tab-pane p-md panel panel-default" data-widget='{"draggable": "false"}'>
         <div class="panel-body profile-photos panel-body">
			<?php $this->load->view('system/sha/application_photos_list');?>
        </div>
    </div>
</div>

<!--Profile Pic Model #STARTS-->
<?php $this->load->view('system/selectProfilePhotoPop');?>
<!--Profile Pic Model #ENDS-->
...
{
	$.ajax({
			url:site_url+'sha/appPagePhotosBoxRefresh',
			type:'POST',
			data:{id:id},
