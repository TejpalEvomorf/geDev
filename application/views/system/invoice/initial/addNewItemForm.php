    ...
    <label class="control-label">Product</label>
        <select class="form-control" id="addNewItem_product" name="product" onchange="productOnChange();" required>
			...
			<?php foreach($products as $sK=>$sV){
					if(in_array($sV['xero_code'],array('42100','42300','43200')))
						continue;
					$pType='0';	
					if($sV['name']=='Airport Pickup Service')
						$pType='apu';
					elseif($sV['name']=='Nomination Fee')
						$pType='nomination';
					elseif($sV['name']=='Placement fee' || $sV['name']=='VIP Placement Fee')
						$pType='placement';	
				?>
                        <option value="<?=$sV['price'].'-'.$sV['gst'].'-'.$sV['id'].'-'.$sV['xero_code'].'-'.$pType?>" ><?=$sV['name']?></option>
                    <?php } ?>
                    <option value="-0" >Overseas Telegraphic Transfer</option>
                    <option value="-0" >Credit Card Surcharge</option>
