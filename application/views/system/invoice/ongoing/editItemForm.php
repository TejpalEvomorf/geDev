...
foreach($items as $item)
{
	if($item['id']==$itemIdArray[1])
	{
		...
		if(in_array($item['xero_code'],array('42100','42300')) && $invoice['study_tour']=='0')
...
?>
      ...
      <div class="m-n form-group col-xs-4" style="padding-left:0;">
        ...
        <input type="text" class="form-control"  id="addNewItem_quantity" value="<?=$item['qty']?>"  name="quantity" required data-parsley-type="number" <?php if($accomodationField){?>readonly="readonly"<?php } ?>>
    </div>
      
      <div class="m-n form-group col-xs-4">
    		<label class="control-label">Quantity unit</label>
        	<select class="form-control" id="addNewItem_qty_unit" name="qty_unit" <?php if($accomodationField || $accomodationFieldStour){?>disabled="disabled"<?php } ?>>
            ...
            <?php if($accomodationField || $accomodationFieldStour){?>
